...
public class AllDirectedPaths<V, E> {
	 ...
	 * @param maxPathLength
	 *            maximum number of edges to allow in a path (if null, all paths
	 *            are considered, which may be very slow due to potentially huge
	 *            output)
	 */
	public List<GraphPath<V, E>> getAllPaths(V sourceVertex, V targetVertex,
			boolean simplePathsOnly, Integer maxPathLength) {
		return getAllPaths(Collections.singleton(sourceVertex),
				Collections.singleton(targetVertex), simplePathsOnly,
				maxPathLength);
	}
	...
	public List<GraphPath<V, E>> getAllPaths(Set<V> sourceVertices,
			Set<V> targetVertices, boolean simplePathsOnly,
		...
		Map<E, Integer> edgeMinDistancesFromTargets = edgeMinDistancesBackwards(
				targetVertices, maxPathLength);

		// Generate all the paths
		List<GraphPath<V, E>> allPaths = generatePaths(sourceVertices,
				targetVertices, simplePathsOnly, maxPathLength,
	 ...
	 *            the target vertices
	 * @param maxPathLength
	 *            maximum number of edges to allow in a path (if null, all edges
	 *            will be considered, which may be expensive)
	 * 
	 * @return the minimum number of edges in a path from each edge to the
	 *         targets, encoded in a Map
	 */
	private Map<E, Integer> edgeMinDistancesBackwards(Set<V> targetVertices,
			Integer maxPathLength) {
		/*
		 * We walk backwards through the network from the target vertices,
		 * marking edges and vertices with their minimum distances as we go.
		 */
	 ...
	 * @param simplePathsOnly
	 *            if true, only search simple (non-self-intersecting) paths (if
	 *            null, all edges will be considered, which may be expensive)
	 * @param edgeMinDistancesFromTargets
	 *            the minimum number of edges in a path to a target through each
	 *            edge, as computed by {@code edgeMinDistancesBackwards}.
	 * 
	 * @return a List of all GraphPaths from the sources to the targets
	 *         satisfying the given constraints
	 */
	private List<GraphPath<V, E>> generatePaths(Set<V> sourceVertices,
			Set<V> targetVertices, boolean simplePathsOnly,
			Integer maxPathLength, Map<E, Integer> edgeMinDistancesFromTargets) {
		/*
		 * We walk forwards through the network from the source vertices,
		 * exploring all outgoing edges whose minimum distances is small enough.
		 */
		...
		for (List<E> incompletePath; (incompletePath = incompletePaths.poll()) != null;) {
			...
			for (E outEdge : graph.outgoingEdgesOf(leafNode)) {
				...
				if (edgeMinDistancesFromTargets.containsKey(outEdge)
						&& ((maxPathLength == null) || ((edgeMinDistancesFromTargets
								.get(outEdge) + lengthSoFar) <= maxPathLength))) {
					List<E> newPath = new ArrayList(incompletePath);
					newPath.add(outEdge);

					// If requested, make sure this path isn't self-intersecting
					if (simplePathsOnly
							&& pathVertices.contains(graph
					...
					if (targetVertices.contains(graph.getEdgeTarget(outEdge))) {
						...
						assert sourceVertices.contains(completePath
								.getStartVertex());
						assert targetVertices.contains(completePath
								.getEndVertex());
