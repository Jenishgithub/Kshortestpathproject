 ...
 * graph changes in between invocations of the
 * {@link #getShortestPath(Object, Object, AStarAdmissibleHeuristic)}
 * getShortestPath} method; no new instance of this class has to be created. The
 * heuristic is implemented using a FibonacciHeap data structure to maintain the
 * set of open nodes. However, there still exist several approaches in
 * literature to improve the performance of this heuristic which one could
 * consider to implement. Another issue to take into consideration is the
 * following: given to candidate nodes, i, j to expand, where f(i)=f(j),
 * g(i)&gt;g(j), h(i)&lt;g(j), f(i)=g(i)+h(i), g(i) is the actual distance from
 * the source node to i, h(i) is the estimated distance from i to the target
 * node. Usually a depth-first search is desired, so ideally we would expand
 * node i first. Using the FibonacciHeap, this is not necessarily the case
 * though. This could be improved in a later version.
 * 
...
public class AStarShortestPath<V, E> {
	 ...
	 *            target vertex
	 * @param admissibleHeuristic
	 *            admissible heuristic which estimates the distance from a node
	 *            to the target node.
	 * 
	...
	private void expandNode(FibonacciHeapNode<V> currentNode, V endVertex) {
		...
		if (graph instanceof UndirectedGraph) {
			...
			outgoingEdges = ((DirectedGraph) graph).outgoingEdgesOf(currentNode
					.getData());
		}

		for (E edge : outgoingEdges) {
			V successor = Graphs.getOppositeVertex(graph, edge,
					currentNode.getData());
			if ((successor == currentNode.getData())
					|| closedList.contains(successor)) { // Ignore self-loops or
															// nodes which have
															// already been
															// expanded
				continue;
			...
			if (!vertexToHeapNodeMap.containsKey(successor)
				...
				double fScore = tentativeGScore
						+ admissibleHeuristic.getCostEstimate(successor,
								endVertex);
				if (!vertexToHeapNodeMap.containsKey(successor)) {
					...
					openList.decreaseKey(vertexToHeapNodeMap.get(successor),
							fScore);
				}
			}
		}
	 ...
	 */
	private GraphPath<V, E> buildGraphPath(V startVertex, V targetVertex,
			double pathLength) {
		List<E> edgeList = this.buildPath(targetVertex);
		return new GraphPathImpl<V, E>(graph, startVertex, targetVertex,
				edgeList, pathLength);
	}
	...
	private List<E> buildPath(V currentNode) {
		if (cameFrom.containsKey(currentNode)) {
			List<E> path = buildPath(Graphs.getOppositeVertex(graph,
					cameFrom.get(currentNode), currentNode));
