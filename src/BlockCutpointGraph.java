 ...
 * @since July 5, 2007
 */
public class BlockCutpointGraph<V, E> extends
		SimpleGraph<UndirectedGraph<V, E>, DefaultEdge> {
	/**
     */
	...
	public BlockCutpointGraph(UndirectedGraph<V, E> graph) {
		...
		this.dfsTree = new SimpleDirectedGraph<V, DefaultEdge>(
				DefaultEdge.class);
		...
		for (Iterator<V> iter = this.cutpoints.iterator(); iter.hasNext();) {
			...
			UndirectedGraph<V, E> subgraph = new SimpleGraph<V, E>(
			...
			for (Iterator<UndirectedGraph<V, E>> iterator = biconnectedSubgraphs
	...
	private void biconnectedComponentFinished(V s, V n) {
		...
		while ((getNumOrder(edge.getSource()) >= getNumOrder(n))
				&& !this.stack.isEmpty()) {
		...
		VertexComponentForbiddenFunction mask = new VertexComponentForbiddenFunction(
				vertexComponent);
		UndirectedGraph<V, E> biconnectedSubgraph = new UndirectedMaskSubgraph<V, E>(
				this.graph, mask);
	...
	private int dfsVisit(V s, V father) {
		...
		for (Iterator<E> iter = this.graph.edgesOf(s).iterator(); iter
				.hasNext();) {
			E edge = iter.next();
	...
	private Set<UndirectedGraph<V, E>> getBiconnectedSubgraphs(V vertex) {
		Set<UndirectedGraph<V, E>> biconnectedSubgraphs = this.vertex2biconnectedSubgraphs
				.get(vertex);
