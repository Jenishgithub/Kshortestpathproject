 ...
 * 
 * <p>
 * The Clique Minimal Separator (CMS) Decomposition is a procedure that splits a
 * graph into a set of subgraphs separated by minimal clique separators, adding
 * the separating clique to each component produced by the separation. At the
...
public class CliqueMinimalSeparatorDecomposition<V, E> {
					...
					for (V z : Graphs.neighborListOf(gprime, y)) {
						if (!reached.contains(z)) {
							...
							if (vertexLabels.get(z) > j) {
								Y.add(z);
								E fillEdge = graph.getEdgeFactory().createEdge(
										v, z);
	 ...
	 * 
	 * @param vertexLabels
	 *            Map that gives a label for each vertex.
	 * 
	 * @return Vertex with the maximal label.
	 */
	private V getMaxLabelVertex(Map<V, Integer> vertexLabels) {
		Iterator<Entry<V, Integer>> iterator = vertexLabels.entrySet()
				.iterator();
	...
	private void computeAtoms() {
		...
		while (iterator.hasNext()) {
			...
			if (generators.contains(v)) {
				Set<V> separator = new HashSet<V>(Graphs.neighborListOf(hprime,
						v));

				if (isClique(graph, separator)) {
					if (separator.size() > 0) {
						if (separators.contains(separator)) {
							fullComponentCount.put(separator,
					...
					ConnectivityInspector<V, E> con = new ConnectivityInspector<V, E>(
							tmpGraph);
	 ...
	 *            the graph.
	 * @param vertices
	 *            the vertices to induce the subgraph from.
	 * 
	...
	private static <V, E> UndirectedGraph<V, E> copyAsSimpleGraph(
		...
		UndirectedGraph<V, E> copy = new SimpleGraph<V, E>(
