...
class BellmanFordIterator<V, E> implements Iterator<List<V>> {
	 ...
	 * Error message.
	 */
	protected final static String NEGATIVE_UNDIRECTED_EDGE = "Negative"
	...
	public List<V> next() {
		...
		if (hasNext()) {
			...
			for (int i = this.prevImprovedVertices.size() - 1; i >= 0; i--) {
				...
				for (Iterator<? extends E> iter = edgesOfIterator(vertex); iter
						.hasNext();) {
					E edge = iter.next();
					V oppositeVertex = Graphs.getOppositeVertex(graph, edge,
							vertex);
	 ...
	 * @return .
	 */
	protected BellmanFordPathElement<V, E> putPrevSeenData(V vertex,
	...
	private BellmanFordPathElement<V, E> createSeenData(V vertex, E edge,
			double cost) {
		BellmanFordPathElement<V, E> prevPathElement = getPrevSeenData(Graphs
				.getOppositeVertex(graph, edge, vertex));

		BellmanFordPathElement<V, E> data = new BellmanFordPathElement<V, E>(
				graph, prevPathElement, edge, cost, epsilon);
	...
	private void relaxVertex(V vertex, E edge) {
		...
		BellmanFordPathElement<V, E> data = createSeenData(vertex, edge,
				shortestPathCost);
	...
	private boolean relaxVertexAgain(V vertex, E edge) {
		...
		// we get the data of the previous pass.
		BellmanFordPathElement<V, E> oppositePrevData = getPrevSeenData(Graphs
				.getOppositeVertex(graph, edge, vertex));
	...
	private void savePassData(List<V> improvedVertices) {
		for (V vertex : improvedVertices) {
			...
			BellmanFordPathElement<V, E> clonedData = new BellmanFordPathElement<V, E>(
					orig);
